...
void EffZoraband_MikauFadeOut(EffZoraband* this, PlayState* play) {
    if (Cutscene_CheckActorAction(play, 527)) {
        if ((EFFZORABAND_GET_F(&this->actor) + 2) ==
            play->csCtx.actorActions[Cutscene_GetActorActionIndex(play, 527)]->action) {
            this->stateFlags |= 2;
        }
        if (play->csCtx.actorActions[Cutscene_GetActorActionIndex(play, 527)]->action == 7) {
