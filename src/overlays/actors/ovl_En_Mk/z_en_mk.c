...
void func_8095954C(EnMk* this, PlayState* play) {
    if (Cutscene_CheckActorAction(play, 127)) {
        Cutscene_ActorTranslateAndYaw(&this->actor, play, Cutscene_GetActorActionIndex(play, 127));

        switch (play->csCtx.actorActions[Cutscene_GetActorActionIndex(play, 127)]->action) {
            case 1:
            case 2:
            case 3:
            case 4:
            case 5:
                func_809592E0(this, play->csCtx.actorActions[Cutscene_GetActorActionIndex(play, 127)]->action - 1);
                break;
        }
    } else {
        func_809592E0(this, 0);
...
void EnMk_Update(Actor* thisx, PlayState* play) {
    ...
    if ((this->unk_27A & 1) && !Cutscene_CheckActorAction(play, 127)) {
